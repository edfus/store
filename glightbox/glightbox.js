(function (global, factory) {
  ...
  function setSize(data, settings) {
    var defaultWith = checkSize(settings.width);
  ...
  var getSourceType = function getSourceType(url) {
    ...
    //   return 'ajax';
    // }
    console.log('url.match(/\.(imglist)$/) is null');
    return 'image';
    //return 'external';
  };
    ...
    _createClass(GlightboxInit, [{
      ...
      value: function preloadSlide(index) {
        ...
        if (type == 'video' || type == 'external') {
          ...
          // }, 200);
          console.log('type == video || type == external');
        } else {
  ...
  Array.from(document.getElementsByClassName('article-entry')).forEach(i => {
    Array.from(i.getElementsByTagName('img')).forEach(e => {
      ...
      if(e.src) {
        ...
        if(glightboxSource.substring(glightboxSource.length - 5, glightboxSource.length) === '.webp' && glightboxSource.substring(glightboxSource.length - 10, glightboxSource.length - 5).indexOf('.') !== -1)
          glightboxSource = glightboxSource.substring(0, glightboxSource.length - 5);
        e.outerHTML = '<a href="' + glightboxSource + '" title="' + e.alt + '" class="glightbox" data-glightbox="' + glightboxCaption + '" rel="View hi-res version">' + e.outerHTML + '</a>';
      }
